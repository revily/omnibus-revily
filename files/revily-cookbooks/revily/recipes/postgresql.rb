...
postgresql_dir = node['revily']['postgresql']['dir']
postgresql_data_dir = node['revily']['postgresql']['data_dir']
postgresql_data_dir_symlink = File.join(postgresql_dir, "data")
postgresql_log_dir = node['revily']['postgresql']['log_directory']
chef_db_dir = Dir.glob("/opt/revily/embedded/service/erchef/lib/chef_db-*").first

user node['revily']['postgresql']['username'] do
  system true
  shell node['revily']['postgresql']['shell']
  home node['revily']['postgresql']['home']
end

directory postgresql_log_dir do
  owner node['revily']['postgresql']['username']
  recursive true
end
...
file File.join(node['revily']['postgresql']['home'], ".profile") do
  owner node['revily']['postgresql']['username']
  mode "0644"
  content <<-EOH
PATH=#{node['revily']['postgresql']['user_path']}
EOH
end
...
if File.directory?("/etc/sysctl.d") && File.exists?("/etc/init.d/procps")
  ...
  template "/etc/sysctl.d/90-postgresql.conf" do
    ...
    owner "root"
    mode  "0644"
    variables(node['revily']['postgresql'].to_hash)
...
execute "/opt/revily/embedded/bin/initdb -D #{postgresql_data_dir}" do
  user node['revily']['postgresql']['username']
...
runit_service "postgresql" do
  down node['revily']['postgresql']['ha']
  control(['t'])
  options({
    :log_directory => postgresql_log_dir,
    :svlogd_size => node['revily']['postgresql']['svlogd_size'],
    :svlogd_num  => node['revily']['postgresql']['svlogd_num']
  }.merge(params))
end

if node['revily']['bootstrap']['enable']
  execute "/opt/revily/bin/revily-ctl start postgresql" do
    retries 20
  end
end
...
pg_port = node['revily']['postgresql']['port']
pg_user = node['revily']['postgresql']['username']
bin_dir = "/opt/revily/embedded/bin"
db_name = "revily"
...
sql_user        = node['revily']['postgresql']['sql_user']
sql_user_passwd = node['revily']['postgresql']['sql_password']
...
sql_ro_user = node['revily']['postgresql']['sql_ro_user']
sql_ro_user_passwd = node['revily']['postgresql']['sql_ro_password']
